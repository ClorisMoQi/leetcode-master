...
func jump(nums []int) int {
    ...
    for i:=0; i<len(nums); i++ {
        if nextCover < nums[i] + i {
            nextCover = nums[i] + i // 更新下一步覆盖最远距离下标
        }
        if i == curCover { //遇到当前覆盖最远距离下标
            if curCover >= len(nums) - 1 { // 如果curCover就能覆盖到nums末尾，步数找到，结束
                break
            }
            steps++ // 不能覆盖到末尾，需要走下一步
            curCover = nextCover // 更新当前覆盖的最远距离的下标
            if nextCover >= len(nums) - 1 { // 如果nextCover覆盖到nums末尾，步数找到，结束
                break
            }
        }
    }
...
func jump(nums []int) int {
    steps := 0 // 记录走的最大步数
    curCover := 0 // 当前覆盖的最远距离下标
    nextCover := 0 // 下一步覆盖的最远距离下标
    for i:=0; i<len(nums)-1; i++ { // 这里是小于len(nums)-1，不加等号（关键所在）
        if nextCover < nums[i] + i {
            nextCover = nums[i] + i // 更新下一步覆盖的最远距离下标
        }
        if i == curCover { // 遇到当前覆盖的最远距离的下标
            curCover = nextCover // 更新当前覆盖的最远距离的下标
            steps++
        }
    }
